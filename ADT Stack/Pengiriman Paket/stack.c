...
*/
	...
	if(!isStackEmpty(s)){	
		p = top(s);	
		while (p != nil){
			printf("%d. %s | %s | %s\n", i, info(p).NamaPengirim, info(p).AlamatPengirim, info(p).AlamatPenerima);
			p = prev(p);
			i++;
		}
	}
...
void ReadPaket(dokumen *data){
	...
	scanf("%s", &data->AlamatPengirim);
	
	printf("No. Hp Pengirim               : "); 
	scanf("%s", &data->NoPengirim);
	...
	scanf("%s", &data->AlamatPenerima);
	
	printf("No. Hp Penerima               : "); 
	scanf("%s", &data->NoPenerima);
}
